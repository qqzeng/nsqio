...
// 同 nsqlookupd.httpServer 类似，参考 nsqlookupd/http.go
...
func (s *httpServer) doConfig(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	// PUT 请求，更新 nsqlookupd TCP addresses 列表
	if req.Method == "PUT" {
		// add 1 so that it's greater than our max when we test for it
		// (LimitReader returns a "fake" EOF)
		readMax := s.ctx.nsqd.getOpts().MaxMsgSize + 1
		...
		s.ctx.nsqd.swapOpts(&opts)           // 原子地更新配置
		s.ctx.nsqd.triggerOptsNotification() // 向 optsNotificationChan push 一条消息，通知 nsqlookupd 的地址已经变更
	}
	// GET 请求获取 nsqlookupd 指定的配置项
