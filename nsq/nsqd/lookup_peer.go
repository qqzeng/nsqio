...
// 关闭 lookupPeer.conn 连接，并更新状态 lookupPeer.state 为 stateDisconnected
...
func (lp *lookupPeer) Command(cmd *nsq.Command) ([]byte, error) {
	...
	if lp.state != stateConnected {
		...
		_, err = lp.Write(nsq.MagicV1) // 在发送正式的命令请求前，需要要先发送一个 4byte 的序列号，用于协定后面用于通信的协议版本
		if err != nil {
			lp.Close()
			return nil, err
		}
		if initialState == stateDisconnected { // 在重新连接成功后，需要执行一个成功连接的回调函数（在正式发送命令请求之前）
	...
	if cmd == nil { // 在创建 lookupPeer 时会发送一个空的命令请求，其目的为创建正式的网络连接，同时，执行连接成功的回调函数
		return nil, nil
	}
	// 发送指定的命令请求到对端（包括命令的 name、params，一个空行以及body（写body之前要先写入其长度））
