...
type Channel struct {
	...
	memoryMsgChan chan *Message							// 内存消息通道。 其关联的 topic 会向此 channel 发送消息，且所有订阅的 client 会开启一个 go routine 订阅此 channel
	...
	// 正在发送中的消息记录集合，直到收到客户端的 FIN 才删除，否则一旦超过 timeout，则重传消息。（因此client需要对消息做去重处理 de-duplicate）
